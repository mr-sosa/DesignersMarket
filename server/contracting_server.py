...
async def submit_transaction(request):
    ...
    #getfromid(sender)
    if(sender!='empresa'):
        return response.json({'error': '{} does not exist'.format(sender)}, status=404)

    if(method_name=='transfer' and kwargs['receiver'] != 'disenador'):
        return response.json({'error': '{} does not exist'.format(kwargs['receiver'])}, status=404)

    k = client.raw_driver.make_key(contract=contract_name, variable='A', args=['empresa'])

    # Get value
    valueE = client.raw_driver.get(k)
